...
from tensorflow.keras.layers import Conv2D, MaxPooling2D, Flatten, Dense, Dropout, Bidirectional, LSTM, Reshape, TimeDistributed, Input
...
def custom_loss(y_true, y_pred):
    return K.mean(categorical_crossentropy(y_true, y_pred, label_smoothing=0.1), axis=-1)
...
# Reshape to match LSTM input shape (3 time steps, 512 features per step)
x = Reshape((3, 2048))(x)  # Assuming 1536 elements after flattening

# LSTM layers
x = Bidirectional(LSTM(128, return_sequences=True, dropout=dropout_var + 0.05))(x)
x = Bidirectional(LSTM(64, return_sequences=True, dropout=dropout_var + 0.05))(x)

# Dense layers (output shape should be (batch_size, 3, num_classes))
x = TimeDistributed(Dense(num_classes, activation='softmax'))(x)

# Define and compile model
model = Model(inputs=inputs, outputs=x)
model.compile(optimizer=Adam(learning_rate=learning_rate, clipnorm=1.0), loss=custom_loss, metrics=['accuracy'])

# Learning rate scheduler and early stopping
reduce_lr = ReduceLROnPlateau(
    monitor='val_loss', 
    factor=0.75,  # Reduces learning rate by 10%
