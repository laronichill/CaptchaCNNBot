...
""" checkpoint = keras.callbacks.ModelCheckpoint(
    'model_epoch_{epoch:02d}.keras',  # Save model every epoch with epoch number in filename
    monitor="val_loss",
    verbose=1,
    save_best_only=False,           # Save model on every epoch, not just the best
    mode="auto",
    save_freq=10                    # Save every epoch
)
hist = model.fit(
    X_train, 
...
callbacks = [reduce_lr]
#Applying the model
hist = model.fit(X_train, [y_train[0], y_train[1], y_train[2]], batch_size=32, epochs=100, validation_split=0.2, callbacks=callbacks)
...
def predict(filepath):
    ...
    if img is not None:
        ...
        try:
            ...
            print("Prediction shape:", [r.shape for r in res])
