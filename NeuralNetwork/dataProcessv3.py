...
datagen = ImageDataGenerator(
    ...
    zoom_range=0.1,
    rotation_range=10,
    brightness_range=[0.8, 1.2],  # New: Random brightness adjustments
...
def custom_loss(y_true, y_pred):
    y_true = K.reshape(y_true, (-1, 3, num_classes))  # Reshape to (batch_size, 3, num_classes)
    y_pred = K.reshape(y_pred, (-1, 3, num_classes))
    loss = K.mean(categorical_crossentropy(y_true, y_pred, label_smoothing=0.1), axis=-1)
    return loss

model = Sequential([
    Conv2D(24, (3, 3), activation='relu', input_shape=(64, 128, 1), kernel_regularizer=l2(0.001)),
    BatchNormalization(),
    MaxPooling2D((2, 2)),
    Conv2D(48, (3, 3), activation='relu', kernel_regularizer=l2(0.001)),
    BatchNormalization(),
    MaxPooling2D((2, 2)),
    ...
    TimeDistributed(Dense(24, activation='softmax'))  # 24 classes for each letter
])
